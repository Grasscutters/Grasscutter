...
| join | join [uid] <多个角色id> | server.join | 均可使用 | 强制入队角色，跟config.json中的avatarLimits有关（跟队内角色数量上限有关）。用法：`join 10000021 10000022` | |
...
| remove | remove <uid> [多个角色在队伍中的序号] | server.remove | 均可使用 | 强制将某个角色从当前队伍中移除。例如`remove 1 2`表示将1号和2号角色移除 |  |
