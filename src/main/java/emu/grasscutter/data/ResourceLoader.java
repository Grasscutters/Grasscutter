...
public class ResourceLoader {
        ...
        if (embryoList == null) {
            ...
            if (playerElementsFile.exists()) {
                try (FileReader fileReader = new FileReader(playerElementsFile)) {
                    GameDepot.setPlayerAbilities(Grasscutter.getGsonFactory().fromJson(fileReader, new TypeToken<Map<String, AvatarConfig>>() {
                    }.getType()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private static void loadOpenConfig() {
        ...
        if (list == null) {
            ...
            java.lang.reflect.Type type = new TypeToken<Map<String, OpenConfigData[]>>() {
            }.getType();
