...
import emu.grasscutter.languages.SupportLanguageEnum;
...
public final class Grasscutter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String arg : args) {
            switch (arg.toLowerCase()) {
                ...
                case "-gachamap" -> {
                    Tools.createGachaMapping("./gacha_mappings.js");
                    return;
                }
            }
        }
    ...
    public static void loadLanguage() {
        String jsonFilePath = String.format("%s%s.json", getConfig().LANGUAGE_FOLDER, config.LocaleLanguage);
        language = SupportLanguageEnum.getSupportLanguageInstance(jsonFilePath, config.LocaleLanguage, () -> {
            SupportLanguageEnum.writeAllSupportLanguageJsonFileToLanguagesDir(config.LANGUAGE_FOLDER);
            // try again
            try {
                return SupportLanguageEnum.getSupportLanguageInstance(new FileReader(jsonFilePath), config.LocaleLanguage);
            } catch (Exception ee) {
                Grasscutter.getLogger().error("Unable to load " + config.LocaleLanguage + ".json");
            }
            return null;
        });
        saveConfig();
