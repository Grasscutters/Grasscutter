...
public class AchievementTriggerEvent implements ITriggerListener {
                ...
                case TRIGGER_FINISH_QUEST_AND:
                    if(IsQuestIdInParam(event.getId(), paramList[0])) {
                        ...
                        if (checkAllQuestFinished(paramList[0], achievementInfo.getParamList()[0])) {
                            achievementInfo.setCurrentProgress(achievementInfo.getTotalProgress());
                            return checkAchievementStatus(achievementInfo);
                        }
                    }
                    break;
    ...
    public boolean checkAchievementStatus(PlayerAchievementInfo achievementInfo){
        player.getAchievementManager().checkAchievement(achievementId, true);
