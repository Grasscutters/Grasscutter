...
public class HandlerMcoinExchangeHcoinReq extends PacketHandler {
    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        ...
        if (session.getPlayer().getCrystals() < exchangeReq.getMcoinCost() && exchangeReq.getMcoinCost() == exchangeReq.getHcoin()) {
            session.send(new PacketMcoinExchangeHcoinRsp(RetcodeOuterClass.Retcode.RETCODE_RET_UNKNOWN_ERROR_VALUE));
            return;
        }
        ...
        session.send(new PacketMcoinExchangeHcoinRsp(RetcodeOuterClass.Retcode.RETCODE_RET_SUCC_VALUE));
    }
}
