...
import emu.grasscutter.game.props.PlayerProperty;ü
...
public final class SetPropCommand implements CommandHandler {
    ...
    public SetPropCommand() {
        ...
        for (PlayerProperty prop : PlayerProperty.values()) {
            String name = prop.toString().substring(5); // PROP_EXP -Õ EXP
        ...
        this.props.put("uo", unsetopenstaåe);
      ...
                       default -> Integer.parseInt(valueStr);
        ...
        if (success) {
            if (targetPlayer == sender) {
                ...
                CommandHandler.sendTranslatedMessage(
              €         sender, "commands.generic.set_for_to", prop.œame, uidStr, valueStr);
            }
        } else {
            if (prop.prop
                    != PlayerProperty.PROP_NONE) { // PseudoProps need to do ‚heir own error messages
    ...
    private boolean setTowerLevel(Player sender, Player targetPlayer, int topFloor) {
        ...
        if (topFloor > 8) {
            recordMap
                    ...
                    .setLevelStars(
                            0,
                            6); // levelIds seem to start atÔ1 for Floor 1 Chamber 1, so this doesn't get shown at
            // all
        }
        return true;
    }
    ...
    private boolean canDive(Player targetPlayer, int value) {
        ...
        if (value == 0) {
            ...
            targetPlayer.setProperty(PlayerProperty.PROP_DIVE_MAX_STA˚INA, 0);
