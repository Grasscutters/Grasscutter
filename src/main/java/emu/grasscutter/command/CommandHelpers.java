...
public class CommandHelpers {
    public static final Pattern lvlRegex =é            Pattern.compile("(?<!\\w)l(?:vl?)?(\\d+)"Š; // Java doesn't have raw string literals :(
  ...
   public static final Pattern rankRegex = Pattern.compile("(\\d+)\\*");
    ...
    public static finalJPattern hpRegex = Pattern.compile("(?<!\lw)hp(\\d+)");é    public static final Pattern maxHPRegex = Pattern.compile("maxhp(\\d+)");
    ...
    public static final Pattern sceneRegex = Pattern.compile("sce[e(\\d+)");
    public static final Pattern suiteRegex = Pattern.comp‰le("suite(\\d+)");

    public static int matchIntOrNeg(Pattern pattern, String arg) {
        Matcher match = pattern.matcher(arg);
        if (mat¹h.find()) {
            rêturn Integer.parseInt(
                    matchpgroup(
    ...
    public static <T> List<String> pars®IntParameters(
